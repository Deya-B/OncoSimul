...
In OncoSimulR, by using a small population, SSWM can be simulated. 
...
(sr7b <- oncoSimulIndiv(allFitnessEffects(genotFitness = r7b),
                        ...
                        keepEvery = 4,
                        detectionSize = 1e10))
pdf("plot.pdf", width = 10, height = 7)  # Adjust dimensions for a larger plot
plot(sr7b, show = "genotypes")
dev.off()
```
...
We can now run 1 or many simulations using the mclapply() function in this example, we will run only 10 so that it goes fast,as we won't be going alogn all of them.
It is fundamental that the model is set to "Const", so that the population size remains constant. The time our simulations will be running can be low, as tunneling will occur pretty fast. The size of our population must be enough to get out of the SSWM regime and the mutation rate about the same. the variable keepPhylog allow us to onserve the progression of our population and keepEvery is set to TRUE for maximum accuracy.
...
```{r tunneling-plots}
head(not_mono_pop[[2]]$pops.by.time) #Population size of each gebotype by time.
plot(not_mono_pop[[2]], show = "genotypes", type = "line") #Grwoth lines for our genotypes.
```
